...
def load_config(args):
    ...
    if args.dataset == 2:
        config_path = "../../configs/lucaflow.yml"
...
def parse_args():
    ...
    parser.add_argument('--dataset', default=1, type=int, help='What dataset to use for training. 1: shu, 0: group dataset, 2: luca')
...
def compute_validation(model, diffusion, dataset, device, residual_op, noise=None):
    ...
    with torch.no_grad():
        for i in range(nsamples):
            ...
            y_pred, e_pred = diffusion.ddpm(x_noise, model, 1000, plot_prog=False)
            mse_loss += (x_noise - e_pred).square().mean()
            res, _ = residual_op(dataset.scaler.inverse(y_pred))
            #res, _ = residual_op(y_pred)
            l1_loss[i] = torch.mean(abs(res))

    model.train()
    return np.mean(l1_loss), mse_loss/nsamples
...
def diffusion_ConFIG_step(model, diffusion, y, scaler, optimizer, loss_m, residual_op, **kargs):
    ...
    x0_pred = (x_t - (1 - a_b).sqrt() * e_pred) / a_b.sqrt()
    eq_residual, _ = residual_op(scaler.inverse(x0_pred))
    #eq_residual, _ = residual_op(x0_pred)
...
def train(config, train_dataset, scaler, args=None):
    ...
    Nx = train_dataset.data.shape[-2]
    Ny = train_dataset.data.shape[-1]
    residual_op = ResidualOp(device=config.device, Nx=Nx, Ny=Ny)
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        if (epoch + 1) % 10 == 0:
            ...
            if args.validation:
                ...
                #val_loss, val_mse = compute_validation(model, diffusion, train_dataset, config.device, residual_op, noise=validation_noise)
                #writer.add_scalar('Val - l1 residual loss', val_loss, epoch)
                epochs.append(epoch)
                train_losses.append((residual_loss / len(train_dataloader)).cpu().detach().item())
                train_mses.append((mse_loss / len(train_dataloader)).cpu().detach().item())
                #val_losses.append(val_loss)
                #val_mses.append(val_mse.cpu().detach().item())
    ...
    plt.plot(epochs, train_mses, label="Training MSE", color="blue")
    #if args.validation:
        #plt.plot(epochs, val_losses, label="Validation Loss", color="orange")
        #plt.plot(epochs, val_mses, label="Validation MSE", color="orange")
    plt.xlabel("Epochs")
    plt.ylabel("Loss")
    plt.yscale("log")
...
def main():
    ...
    #dataset = LucaFlowDataset(data_folder="../../data", size=args.ndata)
